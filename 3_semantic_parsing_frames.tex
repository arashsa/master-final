...
This chapter presents the state-of-the-art in semantic dependency parsing. The goal of semantic dependency parsing can be superficially defined as a representation of `Who did what to whom', and adding `when' and `where' to the equation. This is similar to Semantic Role Labeling (SRL), where the goal is to represent the abstract role that arguments of a predicate can take in the event \cite{Jur:Mar:09}. However, semantic dependency parsing usually has a broader scope in its representation, attempting to add various semantic phenomena, such ass negation, and other scopal dependencies that are not part of the scope of SRL.
...
The SDP 2015 introduced frames, also referred to as sense distinctions, to the DM and PSD target representations. These are added as an extra layer to the sentence where multiple classes are used in order to add specific information on the content words of a sentence. If we examine the semantic dependency graphs in Figure \ref{DM} and \ref{PSD}, we see these classes below the sentence. The number of frames in DM are far lower than PSD, as we can observe in Figure \ref{fig:data}.
...
In Figure \ref{fig:data} we present some high-level statistics on the SD 2015 data set. The data are reproduced from \citeA{Oepen:15}. The PSD representation is the most fine-grained in terms of the linguistic variation of dependency labels. DM and PAS are in this respect more coarse-grained and similar. However, when examining the percentage of trees, projective vs. non-projective graphs, and reentrancies, wee see that PSD is the most `tree-oriented' of the three target representations. 

As we mentioned in our Section \ref{sec:frames}, the number of frames in PSD is approximately 18 times that of the DM data set. 
