...
When we explore dependency grammar, we will for the sake simplicity use the term graph even when we may, strictly speaking, refer to a tree. This is based on two assumptions:
...
Purely deterministic discriminative data-driven models with no need for a grammar were first proposed by \citeA{Kudo:00} and \citeA{Yamada:03}. These models use \textit{Support Vector Machines} (SVMs) for leaning, and rely on their ability to cope with large scale feature spaces. The parser constructs dependency trees in a left-to-right fashion by using three parsing actions: \textit{Shift}, \textit{Right}, and \textit{Left} \cite{Yamada:03}. We have already mentioned that we divide data-driven parsing in two main classes: graph-based and transition-based. The model presented by \citeA{Yamada:03} belongs to the class of transition-based parsers.
